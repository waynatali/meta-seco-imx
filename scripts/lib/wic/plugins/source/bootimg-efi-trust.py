...
class BootimgEFIPlugin(SourcePlugin):
    ...
    def do_prepare_partition(cls, part, source_params, creator, cr_workdir,
        ...
        key_dir = get_bitbake_var("DEPLOY_DIR_IMAGE")
        
        install_cmd = "install -m 0644 %s/%s %s" % \
            (key_dir, "PK.auth", hdddir)
        exec_cmd(install_cmd)
        
        install_cmd = "install -m 0644 %s/%s %s" % \
            (key_dir, "KEK.auth", hdddir)
        exec_cmd(install_cmd)
        
        install_cmd = "install -m 0644 %s/%s %s" % \
            (key_dir, "db.auth", hdddir)
        exec_cmd(install_cmd)
        
        sign_cmd = "sbsign --key %s/db.key --cert %s/db.crt --output %s/%s %s/%s" % \
